...
class LivresController
{
    ...
    public function modidificationLivre($id){
        $livre = $this->livreManager->getLivreById($id);
        require "views/modifierLivre.view.php";
    }

    public function modidificationLivreValidation(){
        ...
        $this->livreManager->modidificationLivreBd($_POST['identifiant'],$_POST['titre'],$_POST['nbPages'],$nomImageToAdd);
        $_SESSION['alert'] = [
            "type" => "success",
    ...
    private function ajoutImage($file, $dir){
        if(!isset($file['name']) || empty($file['name']))
            throw new Exception("Vous devez une image");
        ...
        if(!move_uploaded_file($file['tmp_name'], $target_file))
            throw new Exception("L'ajot de l'image n'as pas fonctionné");
